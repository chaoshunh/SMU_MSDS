...
#Check whether JobRole and JobLevel are independent
cJob <- cbind(talentManage$JobRole, talentManage$JobLevel)
chisq.test(cJob)
```
...
*** Check the likelihood ration test of the nothing model with the forwards model as we will be using forwards model.
*** We get a p-value less than 0.05 so we reject null hypothesis, so our forwards model is better
```{r likelihood ratio test, echo=TRUE}
# Goodness of fit using LRT test
library(lmtest)
lrtest(forward2, nothing)

# Goodness of fit test Pseudo
pR2(forward2)
```
...
## Pearson Studentized residuals and Standard Errors
```{r InfluentialObs, echo=TRUE}
resid(forward2) 
forward2$residuals

# partial residual plot against fitted values (explanatory variable)
plot(residuals(forward2, type="pearson") ~ talentManage$Attrition, main="pearson residual vs Attrition", xlab="Attrition", ylab="Pearson Residual")
...
#plot cooksd 
plot(forward2, which=4, id.n = 5)
plot(forward2, which=5, id.n = 5)
plot(forward2, which=6, id.n = 5)
...
** Checking for predictions using the parameters selected in the above model
```{r data for Analysis, echo=TRUE}
probabilities <- predict(fullModel, type = "response", se=TRUE)
...
talentManage %>%
  ...
  labs(title="Histogram", xlab=~ key, ylab="Count")

#hist.data.frame(talentManage)
