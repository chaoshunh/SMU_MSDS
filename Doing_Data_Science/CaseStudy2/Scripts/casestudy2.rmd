...
talentManage <-read.csv("E:\\Mahesh\\SMU\\GitHub\\SMU_MSDS\\Doing_Data_Science\\CaseStudy2\\data\\casestudy2.csv")  #stringsAsFactors = FALSE)
```
...
```{r}
require(dplyr)
talentManageNew<-talentManage %>% select(Age, Attrition, DistncFrmHm, Education, EnvrnmntSts,Gender, JobLevel,
                                         JobSatsfctn, MonthlyIncm, YersAtCmpny, YrsSncLstPr)
dim(talentManageNew)
```
...
talentManage$Attrition <- factor(talentManage$Attrition, levels=c("No","Yes"), labels=c(0, 1))

#talentManage2 <- talentManage

#talentManage2$Attrition <- as.numeric(talentManage2$Attrition)
#talentManage2$BusinssTrvl <- as.numeric(talentManage2$BusinssTrvl)
#talentManage2$Department <- as.numeric(talentManage2$Department)
#talentManage2$EducatinFld <- as.numeric(talentManage2$EducatinFld)
#talentManage2$Gender <- as.numeric(talentManage2$Gender)
#talentManage2$JobRole <- as.numeric(talentManage2$JobRole)
#talentManage2$MaritalStts <- as.numeric(talentManage2$MaritalStts)
#talentManage2$Over18 <- as.numeric(talentManage2$Over18)
#talentManage2$OverTime <- as.numeric(talentManage2$OverTime)


model <- glm(Attrition~Age+Gender+BusinssTrvl+DailyRate+Department+DistncFrmHm+Education+EducatinFld+EnvrnmntSts+HourlyRate+JobInvlvmnt+JobLevel+JobRole+JobSatsfctn+MaritalStts+MonthlyIncm+MonthlyRate, data=talentManage, family=binomial(link=logit))
summary(model)

# remove over18 column from dataframe
talentManage[, c("Over18", "EmployeeCnt", "EmployeNmbr")] <- list(NULL)

model <- glm(Attrition~., data=talentManage, family=binomial(link=logit))
summary(model)
