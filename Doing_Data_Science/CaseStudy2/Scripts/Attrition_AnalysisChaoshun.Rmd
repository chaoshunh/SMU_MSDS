...
talentManage <-read.csv("/Users/chux/Desktop/SMUclasses/datascience/CaseStudy2/data/casestudy2.csv")
...
from all the histogramm, we found there are eight columns data doesn't show normality. They are
"DailyRate", "MonthlyRate", "Over18", "EmployeeCount", "StandardHours", "EducationField","HourlyRate".
However, we don't kick off these columns at this step.
...
ggplot(talentManage, aes(talentManage$logMthlyInc)) + 
  ...
  labs(x = "Montly income log(US$)", y = "Frequency", title = "Histogram of Log of Monthly Income") +
  theme(plot.title = element_text(hjust = 0.5)) 
...
```{r deeper analysis, echo=TRUE}

talentManage[, c("MonthlyIncm")] <- list(NULL)

# remove column Aggregate satisfaction
talentManage[, c("AggStsfctn")] <- list(NULL)

# columns we used for full model
columns <- colnames(talentManage)
columns
length(columns)

fullModel <- glm(Attrition ~ ., data=talentManage, family=binomial)

summary(fullModel)

nothing <- glm(Attrition ~ 1, data=talentManage, family=binomial(link=logit))
...
dataForAnalysis <- talentManage %>%
  dplyr::select_if(is.numeric) 

cor1 <- cor(data.matrix(talentManage), method=c("kendall"))
corrplot(cor1)
...
cJob <- cbind(talentManage$JobLevel, talentManage$JobRole)
...
forward2 <- glm(Attrition ~ OverTime + JobRole + StckOptnLvl +  EnvrnmntSts +  
    ...
    TtlWrkngYrs + WorkLifBlnc + RltnshpStsf + YrsSncLstPr + logMthlyInc + 
    YrsInCrrntR + EducatinFld + TrnngTmsLsY + Gender + YrsWthCrrMn + 
    DailyRate + Age, data=talentManage, family = binomial(link = logit))
...
ref1 <- as.factor(ifelse(talentManage$Attrition == "Yes", 1, 0))
...
plot(residuals(forward2, type="pearson") ~ talentManage$Attrition, main="Partial residual Plot against fitted Values", xlab="Fitted Values", ylab="Partial Residual Plot")
...
ggplot(forward2, aes(.fitted, .resid)) + 
  ...
  theme_bw()+
  labs(title="Studentized residuals vs Fitted values", xlab="Fitted values", ylab="Studentized residuals")+
  geom_smooth(se=F)

ggplot(forward2, aes(seq_along(.cooksd), .cooksd)) + 
  geom_line(aes(color = Attrition), alpha = .5) +
  theme_bw()+
  labs(title="Cooks D vs observation number", xlab="Observation Nos", ylab="Cooks D values")
...
ggplot(model.data, aes(index, .se.fit)) + 
  geom_point(aes(color = Attrition), alpha = .5) +
  theme_bw()+
  labs(title="Standard errors for Observation", xlab="Observations", ylab="Standard error")
...
ggplot(forward2, aes(.hat, .stdresid)) +
  ...
  labs(title="Residual Vs Leverage", xlab="Leverage", ylab="Studentized residuals")
...
ggplot(forward2, aes(.hat, .cooksd)) +
  ...
  geom_point(aes(color = Attrition), alpha = .5) +
  labs(title="Cooks D distance Vs Leverage", xlab="Leverage", ylab="Cooks D distance")
```
...
ggplot(talentManage, aes(Age, MonthlyIncome, color=Gender)) + 
  geom_point() +
